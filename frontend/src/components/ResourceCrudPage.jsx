...
function ResourceCrudPage({
  ...
  async function handleFormSubmit(payload) {
    ...
    try {
      if (formMode === "create") {
        const postKeys = service.fieldNamesForAction(optionsPayload, "POST");
        let body = { ...payload };
        if (postKeys?.length) {
          body = Object.fromEntries(
            Object.entries(body).filter(([k]) => postKeys.includes(k)),
          );
        }
        ...
        if (patchKeys?.length) {
          body = Object.fromEntries(
            Object.entries(body).filter(([k]) => patchKeys.includes(k)),
          );
        }
  ...
  return (
    <div className="page">
      ...
      <Modal
        ...
        ariaLabel={formMode === "create" ? "Formulario nuevo" : "Formulario edición"}
      >
        {formFields.length === 0 ? (
          <p className="dynamic-form__empty">
            No se pudieron inferir campos. Verifique permisos o cree un registro
            desde el administrador de Django para inicializar el esquema.
          </p>
        ) : (
          <DynamicForm
            ...
            onCancel={closeForm}
            submitLabel={formMode === "create" ? "Guardar" : "Actualizar"}
          />
        )}
      </Modal>
      ...
      >
        <p className="modal__text">
          ¿Eliminar este registro de forma permanente? Esta acción no se puede
          deshacer.
        </p>
      </Modal>
    </div>
